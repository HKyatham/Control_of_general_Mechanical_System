%Luenberger design and simulations for output vector x(t)
...
function [settling_time, sum_estimation_error] = objective(poles, A, B, C, u, t, x0, x_hat0, tolerance)
    ...
    % Simulating the observer system
    A_augmented = A - L1*C;
    B_augmented = [B, L1];
    system_observer = ss(A_augmented, B_augmented, eye(6), zeros(6, 2));
    % input to the observer
    input_observer = [u(:), x_actual(:, 1)];
    [~, ~, x_hat_states_C] = lsim(system_observer, input_observer, t, x_hat0);
    
    % Calculating the estimation error
    error = x_states - x_hat_states_C;
...
% Simulating the observer system for C
system_observer = ss(A_augmented, B_augmented, eye(6), zeros(6, 2));
input_observer = [u(:), x_actual_C(:, 1)];  % Augmented input matrix
[x_hat_C, ~, x_hat_states_C] = lsim(system_observer, input_observer, t, x_hat0);

% Calculating the estimation error
error = x_states_C - x_hat_states_C;
Q = 1000*eye(6);
R = 0.1;
...
sgtitle('Estimation Error for output vector x(t)', 'Interpreter', 'latex');



%% Defining Nonlinear Dynamics Function
...
figure;
plot(t, y(:, 1:6));
sgtitle('Nonlinear System Response with output vector $(x(t))$', 'Interpreter', 'latex');
xlabel('Time (s)');
ylabel('States');
grid on;